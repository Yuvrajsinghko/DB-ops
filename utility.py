...
def signup_entry(user_input):
    ...
    for i in range(user_input):
        ...
        mycursor.execute(
            "INSERT INTO SignupDetails(name,age,gender,username,phone_no,email,user_password,date_joined) VALUES (%s,%s,%s,%s,%s,%s,%s)",
            (name, age, gender, username, phone_numbers, email, password, date.today()))
        mydb.commit()
...
def transport_data_insert(user_input):
    ...
    for i in range(user_input):
        ...
        mycursor.execute(
            ...
            (trans_id, transport_mode, distance, fuel, years_own))
        mydb.commit()
...
def diet_insert(user_input):
    ...
    for k in range(user_input):
        ...
        mycursor.execute("INSERT INTO DietDetails(diet_ref_id,diet_type,meals_per_day,food_source) VALUES(%s,%s,%s,%s)",
                         (diet_ref, d_type, meals, food_s))
    mydb.commit()
...
def lifestylehabits_entry(user_input):
    ...
    for h in range(user_input):
        ...
        mycursor.execute(
            "INSERT INTO LifestyleHabits(life_ref_id,plastic_item_used,shopping_frequency) VALUES(%s,%s,%s)",
            (lif_ref, plastic, shopping))
    mydb.commit()
...
def daily_entry(user_input):
    ...
    for d in range(user_input):
        ...
        mycursor.execute(
            "INSERT INTO DailyEntry(signup_ref_id,transport_ref_id,diet_ref_id,utility_ref_id,lifestyle_ref_id,carbon_score,date_time_of_entry)VALUES (%s,%s,%s,%s,%s,%s)",
            (us_id, tr_id, di_id, uti_id, life_id, carbon, date.today()))
    mydb.commit()
...
def tips_insert(user_input):
    ...
    for p in range(user_input):
        ...
        tip = FAKE.random_element(
            ["turning off lights when not in use", "using public transportation", "carpooling to work",
             "switching to LED bulbs", "recycling plastic and paper", "reducing water usage",
             "avoiding single-use plastics", "planting a tree", "buying local produce", "riding a bicycle",
             "composting organic waste", "bringing reusable bags for shopping"])
...
def create_tab():
    ...
    # 2. Tables referencing SignupDetails
    mycursor.execute("""
